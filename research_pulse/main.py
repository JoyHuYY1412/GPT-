    ...
    def api_queue_bigshot_update(self, conn: sqlite3.Connection, user: sqlite3.Row):
        ...
        prompt = "\n".join(
            [
                ...
                "2. 每个人从 Google Scholar 按时间从新到旧整理最近 5 篇真实论文，写入 payload.recent_papers；每篇给 title、year、venue、一句话介绍、Scholar/论文链接。",
                "3. 每个人再整理 5 篇平均年引用量 > 100 的可核验代表作，写入 payload.influential_papers，并设置 star=true；如果不足 5 篇就只列可核验条目。",
                "4. 标题链接优先用单篇 Google Scholar 详情页；没有详情页时用 Google Scholar title 搜索页，不要虚构 URL。",
                "5. 补一段科研生平：早期做什么、代表性 title、最近 focus 是什么。",
                "6. 按机构分组输出，可用于网页展板。",
                "7. 不要输出“Recent work on ...”“Research direction”等占位论文；没有核验到的 recent_papers 留空，并在 bio/recent_focus 里解释需要后续核验。",
                "8. 不确定信息标注待核验，不要虚构引用量。",
                "",
                "当前关注名单：",
                json.dumps(
                    [
